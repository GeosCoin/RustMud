    ...
    pub trait ServerHandler {
            ...
            loop {
                ...
                match reader.read_line(&mut message) {
                    Ok(_success) => {
                        ...
                        let cur_session: SessionType = (stream.try_clone().unwrap(), addr);
                        let mut is_exist = false;
                        for item in sessions_ok.iter_mut() {
                            println!("{:?} {:?}", item.1, addr);
                            if item.1 == addr {
                                println!("equal");
                                is_exist = true;
                ...
                // let msg = addr.to_string() + ":" + &message;
        ...
        fn send_all(sessions: &mut SessionsType, message: &str) {
            for session in sessions.lock().unwrap().iter_mut() {
                print!("Got message from {:?}: {}", session, message);
